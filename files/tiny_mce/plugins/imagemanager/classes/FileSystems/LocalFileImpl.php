...
class Moxiecode_LocalFileImpl extends Moxiecode_BaseFile {
	 ...
	 */
	function __construct(&$manager, $absolute_path, $child_name = "", $type = MC_IS_FILE) {
...
class _LocalCopyDirTreeHandler extends Moxiecode_FileTreeHandler {
	...
	function __construct(&$manager, $from_file, $dest_file, $handle_as_add_event) {
		$this->_manager = $manager;
...
class _LocalDeleteDirTreeHandler extends Moxiecode_FileTreeHandler {
	...
	function __construct(&$manager, $dir) {
		$this->_manager = $manager;
		$this->_dir = $dir;
		$this->_files = array();
	}
