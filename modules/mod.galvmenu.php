...
class cModGalvmenu
{
    function Init()
    {
        tplLoadTemplateFile('galvmenu.tpl');
    }
	...
	function show()
    {
		...
		$res = $mysqli->query("SELECT * FROM ".TABLE_GFOTOS." WHERE gid IN (SELECT id FROM ".TABLE_TREE." WHERE id=".$mainname->info['id']." OR parent_id=".$mainname->info['id'].") AND tt='gal1' AND visible='1' ORDER BY ordernum DESC");
