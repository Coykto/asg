...
class cModDaily
{
	...
	function show()
    {
		...
		if (!isset($data))
		{
			...
			$cid = array();
			$res = $mysqli->query("SELECT * FROM ".TABLE_ARTICLES." WHERE visible='1' ORDER BY date DESC");
			while ($row = $res->fetch_array())
			{
				$cat = explode(',',$row['cid']);
				$cats = array();
			...
			if (sizeof($cid)>0)
			{
				$res = $mysqli->query("SELECT * FROM article_cats WHERE id IN (".implode(",",$cid).")");
				while ($row = $res->fetch_array())
		...
		{
			$res = $mysqli->query("UPDATE ".TABLE_ARTICLES." SET views=views+1 WHERE id='".$data['id']."' ");
			
			addblock('block2', array(
				'BLOCK2' => get_set('block2')
			));
		
			$res = $mysqli->query("SELECT * FROM ".TABLE_GFOTOS." WHERE gid='".$data['id']."' AND tt='article' AND visible='1' ORDER BY ordernum DESC");
			$cou = $res->num_rows;
			while ($row = $res->fetch_array())
			...
			if ($data['text1']!='') tplParseBlock('open_text1', array(
			...
			tplParseBlock('open', array(
				...
				'TEXT3' => str_replace("../",HOST,str_replace("../upload-files/",HOSTM."/upload-files/",$data['text3'])),
